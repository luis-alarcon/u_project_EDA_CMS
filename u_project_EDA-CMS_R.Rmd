...
<pAs seen on the graph, the majority of the data is under the x-value 
...
#plotting the graphs
p6 = grid.arrange(p2, p3, p4, p5, ncol=2)
...
#We create a series of plots that showcase the total discharges vs average total payments
#This graph show an overall view of all regions
...
average total payments of about 10000; with a constant increase of discharges, 
similar to a horizontal line. The pattern is repeated about 15000  Average 
Total Payments; it is less apparent in the West Region.</p>
...
<p>The following plot reveals a summary generated produced in 
one summarised graph. The graph the relationships between Total Discharges, 
Average Covered Changes, Average Total Payments and Average Medicare Payments, 
including line graphs and scatter plots showing such the relationship.</p>
...
<p>The first graph produced explored the count and distribution of the data per 
region.  We explore further the distribution of the data per region and 
sub-region in the following box plot.</p>
...
<p>We will elaborate further in some of the graphs presented before.
We will start by the scatter plot between Average Covered Charges and 
Average Medicare Payments.</p>

```{r echo=FALSE, Bivariate_Plots_Total_CC-vs-MP_modified,fig.width=9,fig.height=15}
#We create a scatter plot between 
#the average covered charges vs average medicare payments
ggplot(aes(x=Average.Covered.Charges, y=Average.Medicare.Payments, 
           color=Census.Region),data= df_cms) + 
  geom_point(alpha = 1/5, position = position_jitter(h=0))+ 
  ggtitle("Average Covered Charges vs Average Medicare Payments")+ 
  facet_grid(Census.Region.Division ~ Census.Region)+
  theme(text = element_text(size=9),plot.title = element_text(size=8), 
        axis.text=element_text(size=6))
```

<p>The immediate difference that we see is that for the northeast of the USA, 
the two subregions show a different pattern. Division one New England shows a
more narrow distribution with a more linear shape and positive correlation. 
However for Division two Mid-Atlantic the positive correlation is further 
dispersed and not so linear.</p>

<p>The second difference that standouts are that there are differences between 
subdivisions in the South of the USA. Division six and Division seven have 
similarities in shape. However, Division Five has a more steep inclination.</p>

<p>The other two regions have similar shapes and tendencies, in the case of 
the west of the united state, there is an increase in data in Division nine 
against Division eight.</p>

<p>We will elaborate further in some of the graphs presented before. We will 
start by the scatter plot between Total Discharges and 
Average Total Payments.</p>

```{r echo=FALSE, Bivariate_Plots_Total_TD-vs-TP_modified,fig.width=9,fig.height=15}
#We create a scatter plot between 
#the total discharges vs average total payments
ggplot(aes(x=Total.Discharges, y=Average.Total.Payments, 
           color=Census.Region),data= df_cms) + 
  ...
  facet_grid(Census.Region.Division ~ Census.Region)
```
<p>There are no apparent differences, all graphs from all  Regions and Regions 
divisions have similar shape and tendencies. However, there are some divisions 
such as Division two, Division five and Division nine shows a higher number of 
Average Total Payments vs Total Discharges than the other divisions in their 
region.</p>

<p>We will also elaborate further the box plot created that show the count of 
entries per Region and Division. The following Box plot shows similar data but 
showing the box plots per state within those regions and divisions.</p>

```{r,fig.width=9,fig.height=50}
ggplot(aes(x=Census.Region.Division, fill=Census.Region),
       data=df_cms)+
  geom_boxplot(aes(y= Total.Discharges))+
  coord_cartesian(ylim = c(0, 90))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+ 
...
<p>This graph shows that the state of Delaware has the highest 75% percentile 
count of any state in the USA in this dataset.</p>
